...
* **残り**: 他クレートの `members` 追加。`vac-core` は `ShutdownBroker` / runtime path primitives、`vac-motion` は第一段として `MotionFrame` / OSC decode / UDP forwarding helper / OSC / VMC / VRChat encoding helper、`vac-flowgraph` は第一段として Quantity system / instance config を分割済み。
