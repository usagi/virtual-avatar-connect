...
| 10 | `src/web_interface/control/profiles.rs` | 146 | `chrono::Local::now().format("%Y%m%d-%H%M%S").to_string()` | `Zoned::now().strftime("%Y%m%d-%H%M%S").to_string()` | Local → `Zoned::now()` は system tz 利用 |
| 11 | `src/web_interface/control/run_with.rs` | 227 | 同上 | 同上 | |
| 12 | `src/web_interface/control/flowgraph.rs` | 722 | 同上 | 同上 | |
...
- **ログ/ファイル名系 (`profiles.rs` / `run_with.rs` / `flowgraph.rs` / `tts` / `screenshot.rs`) の strftime 結果**: 既存と文字列一致するかを unit test で担保
