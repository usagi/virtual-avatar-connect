...
- [src/web_interface/control/shutdown.rs](../../src/web_interface/control/shutdown.rs) に `POST /api/v1/control/shutdown` を新設し、broker を `ShutdownReason::ControlApi` で trigger するだけの薄い endpoint を置く。
