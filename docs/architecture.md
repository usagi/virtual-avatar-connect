...
| `web_interface` | `state`, `flowgraph`, `bridges`, `ai`, … | Control API がランタイムを操作する **最上位の集約層**のまま。`control/flowgraph/`・`control/table/`・`control/profiles/`・`control/restart/`・`control/run_with/` は `mod` + 補助サブモジュールに分割済み。 |
