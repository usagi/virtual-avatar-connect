...
- 再構造化 Step 7 / R1: `AppCore::boot` / `runtime_handle` / `run` に runner 向け境界を整理。`mod.rs` は `AppCore` 型、runtime handle、run result、`boot.rs` は起動初期化、`server.rs` は actix HTTP サーバ、`cleanup.rs` は shutdown cleanup を担当する。`ShutdownBroker` / `State::new` / bridges・motion・Control API を集約し、将来 `vac-app` runner に移す境界の目印。
