...
- [ ] M-5 docs+flowgraph: 用途拡張の first slice として `flowgraph.vmc.extract_blendshape` と型付き pose / blendshape 出力、`flowgraph.example/vmc-blendshape-trigger` を追加（表情・gesture トリガーの土台）。残りは AI 入力 / 配信制御の具体ノード化。
