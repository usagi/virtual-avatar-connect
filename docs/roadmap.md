...
- [~] Flowgraph subgraph / group（`[[groups]]` metadata + GUI での Create group + node badge 表示 + Inspector 上の group label/color 編集まで完了。engine 側 subgraph 合成と入出力 port 再 export は継続）
