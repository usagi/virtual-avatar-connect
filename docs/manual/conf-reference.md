...
`[[types]]` は `record<schema_id>` 向けの named schema metadata として読み込まれ、`GET /api/v1/control/flowgraph/diagnostics` の `type_schemas[]` に `source_fq`, `id`, `kind`, `description`, `fields` を返します。LF-5n 時点では `kind = "record"` の read-only catalog で、`fields` の値はサーバと同じ socket type parser で検証されます。full structural validation は後続です。
