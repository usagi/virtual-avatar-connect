...
`[[types]]` は `record<schema_id>` 向けの named schema metadata として読み込まれ、`GET /api/v1/control/flowgraph/diagnostics` の `type_schemas[]` に `source_fq`, `id`, `kind`, `description`, `fields` を返します。LF-5m 時点では `kind = "record"` の read-only catalog で、full structural validation は後続です。
