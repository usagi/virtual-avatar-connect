...
class FlowgraphStore {
  ...
  async openFile(fq: string): Promise<void> {
    ...
    try {
      ...
      this.draftNodes = resp.parsed
        ...
        : null;
      this.draftEdges = resp.parsed
        ? resp.parsed.edges.map((e) => ({ from: e.from, to: e.to }))
        : null;
  ...
  updateNodePosition(id: string, x: number, y: number): void {
    ...
    const next = this.draftNodes.map((n) =>
      n.id === id ? { ...n, position: [x, y] as [number, number] } : n,
    );
  ...
  moveGroupBy(groupId: string, dx: number, dy: number): boolean {
    ...
    this.draftNodes = this.draftNodes.map((n, i) => {
      ...
      const pos =
        n.position ??
        ([50 + (i % 6) * 220, 50 + Math.floor(i / 6) * 140] as [
          number,
          number,
        ]);
      return {
        ...n,
        position: [Math.round(pos[0] + dx), Math.round(pos[1] + dy)],
      };
    });
  ...
  duplicateNodes(ids: string[]): boolean {
    ...
    this.#pushHistory(
      sources.length > 1 ? "Duplicate nodes" : "Duplicate node",
    );
  ...
  alignSelectedNodes(axis: "x" | "y"): boolean {
    ...
    const anchor = this.draftNodes.find((n) => selected.has(n.id))
      ?.position ?? [100, 100];
    ...
    this.draftNodes = this.draftNodes.map((n) => {
      ...
      return {
        ...n,
        position: axis === "x" ? [anchor[0], pos[1]] : [pos[0], anchor[1]],
      };
    });
    return true;
  }
  ...
  distributeSelectedNodes(axis: "x" | "y"): boolean {
    ...
    const rows = this.draftNodes
      ...
      .map((n) => ({
        id: n.id,
        position: n.position ?? ([100, 100] as [number, number]),
      }))
      .sort((a, b) =>
        axis === "x"
    ...
    this.#pushHistory(
      axis === "x" ? "Distribute horizontal" : "Distribute vertical",
    );
    const first = rows[0].position;
    const last = rows[rows.length - 1].position;
    const step =
      (axis === "x" ? last[0] - first[0] : last[1] - first[1]) /
      (rows.length - 1);
  ...
  groupSelectedNodes(): boolean {
    ...
    const positions = rows.map(
      (n) => n.position ?? ([100, 100] as [number, number]),
    );
    ...
    this.draftNodes = this.draftNodes.map((n) => {
      ...
      return {
        ...n,
        position: [Math.round(minX + col * 220), Math.round(minY + row * 150)],
      };
    });
    return true;
  }
  ...
  addSelectedNodesToGroup(id: string): boolean {
    ...
    const addIds = this.selectedNodeIds.filter(
      (nodeId) => existingNodes.has(nodeId) && !group.node_ids.includes(nodeId),
    );
    if (addIds.length === 0) return false;
    this.#pushHistory("Add nodes to group");
    this.draftGroups = this.draftGroups.map((g) =>
      g.id === id ? { ...g, node_ids: [...g.node_ids, ...addIds] } : g,
    );
    this.selectedGroupId = id;
    return true;
  }
  ...
  removeSelectedNodesFromGroup(id: string): boolean {
    ...
    const selected = new Set(this.selectedNodeIds);
    const group = this.draftGroups.find((g) => g.id === id);
    if (!group || !group.node_ids.some((nodeId) => selected.has(nodeId)))
      return false;
    ...
    this.draftGroups = this.draftGroups
      .map((g) =>
        g.id === id
          ? {
              ...g,
              node_ids: g.node_ids.filter((nodeId) => !selected.has(nodeId)),
            }
          : g,
      )
  ...
  removeNodeFromGroup(groupId: string, nodeId: string): boolean {
    ...
    this.draftGroups = this.draftGroups
      ...
      .filter((g) => g.node_ids.length > 0);
    if (!this.draftGroups.some((g) => g.id === groupId))
      this.selectedGroupId = null;
    return true;
  }
  ...
  updateGroupLabel(id: string, label: string): boolean {
    ...
    this.draftGroups = this.draftGroups.map((g) =>
      g.id === id ? { ...g, label: nextLabel || null } : g,
    );
    return true;
  }
  ...
  updateGroupColor(id: string, color: string): boolean {
    ...
    this.draftGroups = this.draftGroups.map((g) =>
      g.id === id ? { ...g, color: nextColor || null } : g,
    );
    return true;
  }
  ...
  #pickNewNodePositionNearCentroid(
    ...
    if (nodes.length > 0) {
      ...
      if (withPos.length > 0) {
        const cx =
          withPos.reduce((s, n) => s + n.position![0], 0) / withPos.length;
        const cy =
          withPos.reduce((s, n) => s + n.position![1], 0) / withPos.length;
  ...
  removeNode(id: string): void {
    ...
    if (this.draftGroups) {
      this.draftGroups = pruneGroupsAfterNodeRemoval(
        this.draftGroups,
        new Set([id]),
      );
    }
    if (
      ...
      !this.draftGroups?.some((g) => g.id === this.selectedGroupId)
    ) {
      this.selectedGroupId = null;
    }
  ...
  removeSelection(
    ...
    if (!this.draftNodes || !this.draftEdges)
    ...
    this.#lastDeletion = {
      nodes: removedNodes.map((n) => ({
        ...
        feature: n.feature,
        position: n.position
          ? ([n.position[0], n.position[1]] as [number, number])
          : null,
        properties: { ...n.properties },
      })),
    ...
    this.draftEdges = this.draftEdges.filter(
      (e) =>
        !edgeKeys.has(`${e.from}||${e.to}`) && !edgeMentionsAny(e, nodeSet),
    );
    if (this.draftGroups) {
    ...
    if (this.selectedNodeId && nodeSet.has(this.selectedNodeId))
      this.selectedNodeId = null;
    this.selectedNodeIds = this.selectedNodeIds.filter(
      (id) => !nodeSet.has(id),
    );
   ...
   */
  get isDirty(): boolean {
    if (!this.currentFile?.parsed || !this.draftNodes || !this.draftEdges)
      return false;
    ...
    const baseEdges = JSON.stringify(
      base.edges.map((e) => ({ from: e.from, to: e.to })),
    );
    const draftEdges = JSON.stringify(
      this.draftEdges.map((e) => ({ from: e.from, to: e.to })),
    );
    if (baseEdges !== draftEdges) return true;
    const baseGroups = JSON.stringify((base.groups ?? []).map(normalizeGroup));
    const draftGroups = JSON.stringify(
      (this.draftGroups ?? []).map(normalizeGroup),
    );
  ...
  async saveCurrent(): Promise<boolean> {
    ...
    this.mutating = true;
    try {
      const resp = await api.flowgraphPutFile(this.currentFq, {
        content: nextToml,
      });
      this.mutating = false;
  ...
  async openExternal(fq: string): Promise<void> {
    try {
      ...
      if (resp.spawned)
        toastStore.info("外部エディタで開きました", resp.command);
  ...
  async reloadFromDisk(): Promise<void> {
    try {
      ...
      if (resp.ok) toastStore.success("Reloaded", `${resp.node_count} ノード`);
        ...
        toastStore.warn("Reloaded (診断あり)", `${resp.diagnostics.length} 件`);
      await Promise.all([this.refreshTree(), this.refreshDiagnostics()]);
  ...
  ): Promise<FragmentCopyResponse | null> {
    try {
  ...
  async fragmentCopySelectedNode(): Promise<FragmentCopyResponse | null> {
    ...
    return this.fragmentCopy({
      scope: "nodes",
      targets: [
        { kind: "node", fq_file: this.currentFq, node_id: this.selectedNodeId },
      ],
      origin: this.currentFq,
    });
  }
  ...
  ): Promise<FragmentPasteResponse | null> {
    this.mutating = true;
    try {
  ...
  #userEnumPaletteSpecs(): FlowgraphNodeSpec[] {
    ...
    const base = this.catalog?.specs.find(
      (s) => s.feature === "flowgraph.literal.string",
    );
    if (!base) return [];
  ...
  attachWsSubscriber(): void {
    ...
    this.#wsUnsubscribe = eventsStore.subscribe((ts) => {
      ...
      if (ev.ok) {
        ...
        toastStore.warn(
          "Flowgraph reload に診断",
          `err=${ev.error_count} warn=${ev.warning_count}`,
        );
      }
...
function normalizeGroup(
  g: FlowgraphGroupDef | FlowgraphDraftGroup,
): FlowgraphDraftGroup {
  return {
    id: g.id,
    label: g.label ?? null,
...
function pruneGroupsAfterNodeRemoval(
  groups: FlowgraphDraftGroup[],
  ...
  return groups
    .map((g) => ({
      ...g,
      node_ids: g.node_ids.filter((id) => !removedNodeIds.has(id)),
    }))
    .filter((g) => g.node_ids.length > 0);
}
...
function edgeMentionsNode(edge: FlowgraphDraftEdge, nodeId: string): boolean {
  return (
    parsePortRef(edge.from).nodeId === nodeId ||
    parsePortRef(edge.to).nodeId === nodeId
  );
}

/** γ-4a.0: 複数ノード版。`removeSelection` の巻き添えエッジ計算に使う。 */
function edgeMentionsAny(
  edge: FlowgraphDraftEdge,
  nodeIds: Set<string>,
): boolean {
  return (
    nodeIds.has(parsePortRef(edge.from).nodeId) ||
    nodeIds.has(parsePortRef(edge.to).nodeId)
  );
}
...
export function serializeFlowgraph(doc: {
  ...
  if (hasMeta && m) {
    ...
    if (m.library_uses && m.library_uses.length > 0) {
      lines.push(
        `library_uses = [${m.library_uses.map(tomlString).join(", ")}]`,
      );
    }
    lines.push("");
  }
  ...
  for (const node of doc.nodes) {
    ...
    if (node.position) {
      lines.push(
        `position = [${formatNum(node.position[0])}, ${formatNum(node.position[1])}]`,
      );
    }
...
function tomlValue(v: unknown): string {
  ...
  if (typeof v === "number")
    return Number.isInteger(v) ? String(v) : formatNum(v);
  if (Array.isArray(v)) {
...
export function summarizeDiagnostics(
  diags: FlowgraphDiagnostic[] | undefined,
): {
  error: number;
  warning: number;
  info: number;
...
function describeError(e: unknown): string {
  if (e instanceof ControlApiError) {
    const detail =
      ...
      e.body &&
      "error" in (e.body as Record<string, unknown>)
