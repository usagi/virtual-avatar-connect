<script lang="ts">
  ...
  const summary = $derived(
    summarizeDiagnostics(flowgraphStore.diagnostics?.diagnostics),
  );
  ...
  const studioCommands: StudioCommand[] = $derived([
    ...
    },
    {
      ...
      disabled:
        ...
        flowgraphStore.selectedNodeIds.length === 0,
    ...
    },
    {
      ...
      label: "Create group",
      description:
        "Save selected nodes as a visual Flowgraph group and pack them into a compact layout.",
      disabled: flowgraphStore.selectedNodeIds.length < 2,
    ...
    },
    {
      ...
      run: () => {
        if (flowgraphStore.selectedGroupId)
          flowgraphStore.selectGroupNodes(flowgraphStore.selectedGroupId);
      },
    },
    {
      id: "remove_group",
      ...
      description:
        "Remove the selected visual group without deleting its member nodes.",
      disabled: !flowgraphStore.selectedGroupId,
      run: () => {
        if (flowgraphStore.selectedGroupId)
          flowgraphStore.removeGroup(flowgraphStore.selectedGroupId);
      },
    },
    {
      id: "reload",
      ...
      description:
        "Refresh Flowgraph files and diagnostics from the configured root.",
    ...
    },
    {
      ...
      label: "Copy fragment",
      description:
        "Copy the selected node, or the current file when no node is selected.",
      disabled: !flowgraphStore.currentFq,
      run: onCopy,
    },
    {
  ...
  function onUndo() {
    if (flowgraphStore.undo())
      toastStore.info("Undo", flowgraphStore.undoStack.at(-1)?.label ?? "");
  }

  function onRedo() {
    if (flowgraphStore.redo())
      toastStore.info("Redo", flowgraphStore.redoStack.at(-1)?.label ?? "");
  }
  ...
  function onDuplicateSelection() {
    ...
    if (ok)
      toastStore.success(
        "Duplicated",
        `${flowgraphStore.selectedNodeIds.length} node(s)`,
      );
  }
  ...
  async function onOpenExternal() {
    if (flowgraphStore.currentFq)
      await flowgraphStore.openExternal(flowgraphStore.currentFq);
  }

  async function onCopy() {
    // 選択中ノードがあればそれを、なければ現在ファイル全体を fragment 化。
    if (flowgraphStore.selectedNodeId)
      await flowgraphStore.fragmentCopySelectedNode();
...
<div class="flex h-[calc(100vh-9rem)] min-h-[42rem] flex-col gap-3">
  <div class="flex flex-wrap items-start justify-between gap-3">
    <div>
      ...
      <p class="text-sm opacity-65">
        Author, inspect, and repair runtime dataflow files.
      </p>
    </div>
  ...
  <div class="vac-panel-muted flex flex-wrap items-center gap-2 px-3 py-2">
    <div class="flex items-baseline gap-2">
      ...
      <span class="text-xs opacity-60">
        ...
        {#if flowgraphStore.tree && !flowgraphStore.tree.exists}
          <span class="ml-1 rounded bg-warning-500/30 px-1 text-warning-900-100"
            >missing</span
          >
        {/if}
      </span>
    ...
    <label
      class="flex items-center gap-1 text-[10px] opacity-70"
      title="Resize file pane"
    >
      Files
      <input
        class="w-20"
    ...
    >
      ...
      <input
        ...
        max="520"
        step="10"
        bind:value={inspectorPaneWidth}
      />
    </label>
    <label
      class="flex items-center gap-1 text-[10px] opacity-70"
      title="Resize problems panel"
    >
      Bottom pane
      <input
        class="w-20"
  ...
  {#if commandPaletteOpen}
    ...
    >
      ...
      >
        <div
          class="vac-panel-header flex items-center justify-between px-4 py-3"
        >
          <div>
            <h3
              id="flowgraph-command-palette-title"
        ...
        <div class="grid gap-1 p-2">
          ...
          {:else}
            <div class="px-3 py-8 text-center text-sm opacity-60">
              No matching command.
            </div>
          {/each}
        </div>
  ...
  <!-- Studio workspace -->
  <div
    class="grid min-h-0 flex-1 gap-2 xl:grid-cols-[260px_minmax(34rem,1fr)_320px]"
    style={workspaceGridStyle}
  >
    <section class="vac-panel flex min-h-[16rem] flex-col overflow-hidden">
      <div
        class="vac-panel-header px-3 py-2 text-xs font-semibold uppercase tracking-wider opacity-60"
      >
        Files
      </div>
    ...
    <section class="vac-panel flex min-h-[28rem] flex-col overflow-hidden">
      <div
        class="vac-panel-header flex items-center justify-between gap-2 px-3 py-2"
      >
        <span class="text-xs font-semibold uppercase tracking-wider opacity-60"
          >Canvas</span
        >
        <span class="truncate text-xs opacity-60"
          >{flowgraphStore.currentFq ?? "No file selected"}</span
        >
      </div>
    ...
    <div
      class="grid min-h-[28rem] grid-rows-[minmax(12rem,1fr)_minmax(12rem,1fr)] gap-2"
    >
