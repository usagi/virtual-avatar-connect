<script lang="ts">
  ...
  function socketTypeExprCompatible(
    ...
    return (
      outArgs.length === inArgs.length &&
      outArgs.every((arg, index) =>
        socketTypeExprCompatible(arg, inArgs[index]),
      )
    );
  }
  ...
  function portsWireCompatible(
    ...
    let ok =
      outExpr && inExpr ? socketTypeExprCompatible(outExpr, inExpr) : false;
    if (!outExpr || !inExpr) {
      if (outTy === inTy) ok = true;
      else if (outTy === "json" || inTy === "json") ok = true;
      else if (
        (outTy === "float" && inTy === "quantity") ||
        (outTy === "quantity" && inTy === "float")
      )
        ok = true;
      else if (outTy === "quantity" && inTy === "string") ok = true;
      else if (
        (outTy === "string" && inTy === "datetime") ||
        (outTy === "datetime" && inTy === "string")
      )
        ok = true;
    }
    if (!ok) return false;
  ...
  function toFlowNodes(): Node[] {
    ...
    return draft.map((n, i) => {
      ...
      return {
        ...
        selected:
          flowgraphStore.selectedNodeIds.includes(n.id) ||
          flowgraphStore.selectedNodeId === n.id,
      };
    });
  }
  ...
  function toFlowEdges(): Edge[] {
    ...
    return draftEdges.map((e, i) => {
      ...
      if (isExec) style = "stroke: rgb(249 115 22); stroke-width: 2;";
        ...
        style =
          "stroke: rgb(239 68 68); stroke-width: 2; stroke-dasharray: 5 4;";
      return {
        id: `e-${i}-${e.from}->${e.to}`,
        source: from.nodeId,
  ...
  function findPortSpec(
    ...
    const spec: FlowgraphNodeSpec | undefined = flowgraphStore.findSpec(
      node.feature,
    );
  ...
  function onNodeDragStop(params: {
    ...
    if (isGroupFrameNode(n.id)) {
      const currentFrame = toGroupFrameNodes().find(
        (frame) => frame.id === n.id,
      );
  ...
  function onDelete(params: { nodes: Node[]; edges: Edge[] }) {
    ...
    const nodeIds = params.nodes
      .map((n) => n.id)
      .filter((id) => !isGroupFrameNode(id));
    const edgePairs = params.edges
      .map(
        (e) =>
          (e.data as { original?: { from: string; to: string } } | undefined)
            ?.original,
      )
      .filter((x): x is { from: string; to: string } => !!x);
    const { removedNodes, removedEdges } = flowgraphStore.removeSelection(
      nodeIds,
      edgePairs,
    );
  ...
  const nodeTypes = {
    flowgraph: FlowgraphNodeCard,
    flowgraphGroup: FlowgraphGroupFrame,
  };
</script>
...
<div class="relative h-full w-full">
  ...
  {:else if !flowgraphStore.currentFile?.parsed}
    <div
      class="flex h-full items-center justify-center px-4 text-center text-sm opacity-70"
    >
      <div>
        <p class="mb-2 font-semibold text-error-500">TOML パース失敗</p>
        <p class="text-xs">
          外部エディタでファイルを修正してください。右上の「Open
          external」から開けます。
        </p>
      </div>
    </div>
  {:else}
    ...
    >
      ...
      <FlowgraphAutoFit
        triggerKey={`${flowgraphStore.currentFq ?? ""}::${nodes.length}`}
      />
    </SvelteFlow>
    <!-- γ-4a.0: 削除操作の発見性確保。編集中キャンバスの左下にキーボードヒントを固定表示。 -->
    <div
      class="flowgraph-keyhint pointer-events-none absolute bottom-1 left-1 select-none"
    >
      <code>Delete</code> / <code>Backspace</code> : 選択削除 ・
      <code>Ctrl+D</code>
      : 複製 ・ <code>Ctrl+Click</code> : 複数選択
    </div>
  {/if}
</div>
