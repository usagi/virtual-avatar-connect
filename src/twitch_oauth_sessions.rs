...
#[derive(Debug)]
pub(crate) struct OAuthSessionInternal {
	pub(crate) account: OAuthAccount,
	pub(crate) user_code: String,
	pub(crate) verification_uri: String,
	pub(crate) interval_secs: u64,
	pub(crate) expires_at_system: SystemTime,
	pub(crate) started_at_system: SystemTime,
	pub(crate) expires_at_instant: TokioInstant,
	pub(crate) status: OAuthSessionStatus,
	pub(crate) last_error: Option<String>,
	/// Handle to the polling task so `cancel` can abort it.
	pub(crate) cancel_handle: Option<JoinHandle<()>>,
}

impl OAuthSessionInternal {
	pub(crate) fn to_view(&self) -> OAuthSessionView {
		OAuthSessionView {
...
pub struct OAuthSessions {
	pub(crate) inner: RwLock<HashMap<OAuthAccount, OAuthSessionInternal>>,
}
