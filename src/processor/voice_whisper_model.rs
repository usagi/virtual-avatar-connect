...
fn preset_to_filename(preset: &str) -> Option<&'static str> {
 ...
 if p.eq_ignore_ascii_case("large-v3")
  || p.eq_ignore_ascii_case("large_v3")
  || p.eq_ignore_ascii_case("large")
 {
...
pub(crate) fn resolve_whisper_model(spec: &str) -> Result<PathBuf, String> {
 ...
 if path.is_file() {
  return path
   .canonicalize()
   .map_err(|e| format!("モデルパスの canonicalize に失敗: {e}"));
 }

 let looks_like_path = s.contains('/') || s.contains('\\') || is_windows_drive_path(s);
 if looks_like_path {
  return Err(format!(
   "Whisper モデルファイルが見つかりません: {}",
   s
  ));
 }
