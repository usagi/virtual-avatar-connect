...
use whisper_rs::{install_logging_hooks, FullParams, SamplingStrategy, WhisperContext, WhisperContextParameters, WhisperState};
	...
	let join_handle = std::thread::Builder::new()
		...
		.spawn(move || {
			...
			match run_whisper_loop(
				...
				Ok(()) => {}
				Err(e) => log::error!("《Flowgraph/Voice》: Whisper スレッドが終了しました: {} ({})", e, source_label),
			}
		})
		.ok()?;
...
fn whisper_lang_primary(s: &str) -> String {
	...
	t.split(|c| c == '-' || c == '_').next().unwrap_or("ja").to_string()
}
...
fn run_whisper_loop(
	...
	log::info!("《Voice》: 入力 {:} Hz, {} ch, format={:?}", in_hz, channels, sample_format);
	...
	loop {
		if stop_flag.load(Ordering::Relaxed) {
			log::info!(
				"《Voice》: Whisper ループに shutdown 要求を受信したため終了します ({})",
				source_label
			);
			break Ok(());
		}
