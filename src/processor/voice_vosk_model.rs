...
pub(crate) fn resolve_vosk_model_dir(spec: &str) -> Result<PathBuf, String> {
...

 {
  ...
  let mut archive =
   zip::ZipArchive::new(reader).map_err(|e| format!("zip 読み取り: {e}"))?;
  archive
   .extract(&cache_root)
   .map_err(|e| format!("zip 展開: {e}"))?;
 }
