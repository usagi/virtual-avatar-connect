...
pub async fn resolve_speaker_style_id(
 engine_base: &str,
 pc: &ProcessorConf,
 ctx: &str,
) -> Result<i64> {
...
pub async fn fetch_global_style_id(
 engine_base: &str,
 speaker_uuid: &str,
 local_index: usize,
 ctx: &str,
...
pub async fn fetch_speakers_list(engine_base: &str, ctx: &str) -> Result<Vec<VoicevoxSpeaker>> {
 ...
 let arr = speakers_json_as_array(&v).context(format!(
  "{}: /speakers の JSON を解釈できませんでした",
  ctx
 ))?;
 let mut out = Vec::with_capacity(arr.len());
 for (i, item) in arr.iter().enumerate() {
  let s: VoicevoxSpeaker = serde_json::from_value(item.clone()).with_context(|| {
   format!(
    "{}: speakers[{}] を VOICEVOX 互換形式としてパースできませんでした",
    ctx, i
   )
  })?;
  out.push(s);
