...
pub async fn delete_file(client: &Client<OpenAIConfig>, file_id: &str) -> Result<()> {
	for n in 1..11 {
		match client.files().delete(file_id).await {
			Ok(res) => {
				log::info!(
					"アップロードしたファイルの削除が完了しました: file_id={:?} deleted={}",
					res.id,
					res.deleted
				);
				return Ok(());
...
pub async fn delete_files(client: &Client<OpenAIConfig>, train_file_id: String, validation_file_id: Option<String>) -> Result<()> {
	...
	loop {
		...
		match res.status {
			...
			FineTuningJobStatus::Cancelled => {
				let m = format!(
					"Fine-tuning はキャンセルされました({}経過): job_id={:?}",
					fmt_elapsed(t.elapsed()),
					job_id
				);
				log::error!("{}", m);
