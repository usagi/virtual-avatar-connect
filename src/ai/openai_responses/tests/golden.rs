...
use crate::ai::openai_responses::types::{
	CreateResponseRequest, ErrorObject, InputContent, InputItem, MessageContent, OutputItem, Reasoning, ReasoningEffort, Response,
	ResponseStatus, TextConfig, TextFormat, Tool, ToolChoice, ToolChoiceMode, Usage,
};
...
fn deserialize_response_reasoning_without_encrypted_content() {
	...
	match &res.output[0] {
		OutputItem::Reasoning { id, encrypted_content, .. } => {
			...
			assert!(encrypted_content.is_none(), "include 指定なしなら encrypted_content は None");
		}
		_ => panic!("expected Reasoning"),
	}
}
...
fn serialize_input_reasoning_skips_none_fields() {
	...
	assert!(v.get("encrypted_content").is_none(), "None encrypted_content must be skipped");
