...
use super::types::stream::{
	ErrorEnvelope, FunctionCallArgumentsDeltaPayload, FunctionCallArgumentsDonePayload, OutputItemEnvelope, OutputTextDeltaPayload,
	OutputTextDonePayload, ResponseEnvelope, StreamEvent,
};
...
fn parse_typed_value(event_type: &str, value: serde_json::Value, raw_data: &str) -> Result<StreamEvent, SseError> {
	...
	Ok(match event_type {
		...
		"response.output_text.delta" => {
			let p: OutputTextDeltaPayload = serde_json::from_value(value).map_err(|e| err(event_type, e))?;
		...
		"response.output_text.done" => {
			let p: OutputTextDonePayload = serde_json::from_value(value).map_err(|e| err(event_type, e))?;
		...
		"response.function_call_arguments.delta" => {
			let p: FunctionCallArgumentsDeltaPayload = serde_json::from_value(value).map_err(|e| err(event_type, e))?;
		...
		"response.function_call_arguments.done" => {
			let p: FunctionCallArgumentsDonePayload = serde_json::from_value(value).map_err(|e| err(event_type, e))?;
