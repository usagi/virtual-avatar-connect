//! Chat Completions / Responses へのリクエストで **モデル系列ごとに異なる既定** を一箇所にまとめる。
...
/// [`apply_model_chat_options`] の Responses 版。
///
...
/// - `text.format`: 旧 `ResponseFormat::Text` の Responses 等価。Responses の既定は
///   そもそも自由文なので明示は不要だが、gpt-5 系で structured outputs の誤起動を防ぐため
///   `TextFormat::Text` を明示する（Chat Completions 時代の挙動を維持）。
///
/// いずれも「上書きする」ではなく「未指定なら埋める」セマンティクスではなく、
/// persona 設定の値で **常に** 上書きする（builder の role を踏襲）。
pub(crate) fn apply_model_responses_options(
 ...
 if let Some(effort) = reasoning_effort {
  if is_gpt5_family(Some(model)) {
   ...
   log::warn!(
    "openai_reasoning_effort は gpt-5 系モデル専用です（指定モデル: {model}）。無視します。"
   );
  }
 }
}
