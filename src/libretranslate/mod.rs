...
const PYTHON_EMBED_ZIP_TEMPLATE: &str =
...
impl Runtime {
 ...
 pub async fn ensure_server(
  ...
  #[cfg(not(windows))]
  {
   let _ = port;
   Err(
    "LibreTranslate の自動起動（embed）は現在 Windows のみです。\
...
async fn prepare_windows_embedded_python() -> Result<PathBuf, String> {
 let root = cache_root();
 let ver = std::env::var("VAC_LIBRETRANSLATE_EMBED_PYTHON_VERSION")
  .unwrap_or_else(|_| EMBED_PYTHON_VERSION.to_string());
 ...
 if pip_ok.is_file() && python_exe.is_file() {
  log::info!(
   "《LibreTranslate》: 埋め込み Python を再利用します {:?}",
   embed_dir
  );
  return Ok(embed_dir);
 }
 ...
 log::info!(
  "《LibreTranslate》: pip で libretranslate をインストールします（初回は数分かかることがあります）"
 );
...
fn fix_embed_pth(embed_dir: &Path) -> Result<(), String> {
 ...
 let mut s = std::fs::read_to_string(&pth)
  .map_err(|e| format!("{} 読み取り: {}", pth.display(), e))?;
...
#[cfg(windows)]
async fn start_libretranslate_process(
 embed_dir: &Path,
 port: u16,
...
fn extract_zip_flat_or_rooted(data: &[u8], dest: &Path) -> Result<(), String> {
 ...
 let mut archive =
  zip::ZipArchive::new(reader).map_err(|e| format!("zip: {e}"))?;
