...
pub(crate) fn resolve_twitch_client_id(es: &TwitchEventSubConfig) -> String {
	std::env::var(ENV_CLIENT_ID)
		...
		.or_else(|| es.client_id.as_ref().map(|s| s.trim().to_string()).filter(|s| !s.is_empty()))
...
impl OAuthIdent {
	...
	pub fn for_key(key: &str, es: &TwitchEventSubConfig, spec: Option<&TwitchTokenKeySpec>) -> Self {
		...
		let timeout_secs = spec.and_then(|s| s.oauth_timeout_secs).unwrap_or_else(|| {
			if key == "broadcaster" {
...
pub(crate) async fn poll_device_token(
	...
	loop {
		if tokio::time::Instant::now() >= deadline {
			return Err(anyhow!(
				"device 認可がタイムアウトしました（ユーザーが期限内に Twitch でコードを入力しませんでした）。"
			));
		}
		...
		if status.is_success() {
			let tr: TokenResponse = serde_json::from_str(&text).with_context(|| format!("token JSON の解析に失敗しました: {}", text))?;
			return Ok(tr);
		}
		...
		if msg.eq_ignore_ascii_case("authorization_pending") {
			log::debug!(
				"《Twitch》 OAuth: 認可待ち（authorization_pending）…{} 秒後に再試行します。",
				interval.as_secs()
			);
			wait = interval;
			continue;
		...
		if msg.eq_ignore_ascii_case("slow_down") {
			...
			log::debug!(
				"《Twitch》 OAuth: slow_down 指示を受けたため間隔を {} 秒に伸ばします。",
				wait.as_secs()
			);
			continue;
		}
				...
				match refresh_access_token(&ident.client_id, rt).await {
					Ok(tr) => {
						...
						if let Err(e) = save_stored_tokens(ident, &new_stored) {
							...
							log::info!("《Twitch》 OAuth[{}]: refresh によりアクセストークンを更新しました。", ident.label);
						}
...
pub(crate) async fn ensure_moderator_access_token(es: &TwitchEventSubConfig, mc: &TwitchModeratorConfig) -> Result<String> {
	if !mc.oauth_auto
		&& mc.user_access_token.as_deref().filter(|s| !s.trim().is_empty()).is_none()
