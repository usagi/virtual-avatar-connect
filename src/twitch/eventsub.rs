...
async fn run_eventsub_session(state: &SharedState, resolved: &mut ResolvedEventSub) -> Result<()> {
 ...
 let welcome_str: String = loop {
  ...
  match next {
   ...
   Message::Close(frame) => {
    return Err(anyhow!(
     "EventSub: welcome 前にサーバかめEClose されました: {:?}",
     frame
    ));
   },
   other => {
    log::debug!(
     "EventSub: skipping non-text frame: {:?}",
...
async fn create_subscriptions(client_id: &str, bearer: &str, session_id: &str, r: &ResolvedEventSub) -> Result<()> {
 ...
 if r.cfg.raid {
  queue.push((
   "channel.raid",
   "1",
   serde_json::json!({ "to_broadcaster_user_id": bid }),
  ));
 }
 ...
 if r.cfg.channel_points {
  ...
  if reward_ids.is_empty() {
   ...
   if r.cfg.channel_points_reward_id.as_ref().map(|s| s.trim().is_empty()).unwrap_or(true) {
    log::info!(
     "EventSub: subscribing channel_points for {} reward(s)",
     reward_ids.len()
    );
   }
...
pub(crate) fn default_subscription_queue_from_cfg(cfg: &TwitchEventSubConfig, broadcaster_id: &str) -> Vec<(&'static str, &'static str, Value)> {
...
pub(crate) fn explicit_subscription_queue(event_types: &[String], broadcaster_id: &str) -> Vec<(String, &'static str, Value)> {
 ...
 for raw in event_types {
  ...
  let entry = match t {
   "stream.online" | "stream.offline" | "channel.subscribe" | "channel.subscription.message"
...
fn format_notification(sub_type: &str, event: &Value) -> String {
 match sub_type {
  ...
  "stream.offline" => {
   let n = event["broadcaster_user_name"].as_str().or(event["broadcaster_user_login"].as_str()).unwrap_or("?");
