  ...
  let (command, if_not_running, run_as_admin, working_dir, minimized) = match run_with {
   ...
   if is_http_url {
    ...
    #[cfg(target_os = "windows")]
    {
     ...
     use windows::Win32::UI::WindowsAndMessaging::{SW_SHOWNORMAL, SW_SHOWMINIMIZED};
    ...
    if run_as_admin {
     ...
     #[cfg(target_os = "windows")]
     {
      if minimized {
       log::info!(
        "run_with: 管理者権限・最小化のため ShellExecuteW (runas) を使います（プロセス終了は待機しません）。"
       );
...
#[cfg(target_os = "windows")]
fn run_with_minimize_top_level_windows_for_pids(
 ...
 use windows::Win32::UI::WindowsAndMessaging::{
  EnumWindows, GetWindow, GetWindowRect, GetWindowThreadProcessId, GW_OWNER, IsWindowVisible,
  WNDENUMPROC,
 };
 ...
 unsafe {
  ...
  EnumWindows(enum_fn, LPARAM(ptr as isize)).map_err(|e| {
   std::io::Error::new(std::io::ErrorKind::Other, format!("EnumWindows: {}", e))
  })?;
...
fn run_with_apply_minimize_to_hwnds(hwnds: &[windows::Win32::Foundation::HWND]) {
 ...
 use windows::Win32::UI::WindowsAndMessaging::{
  PostMessageW, ShowWindow, WM_SYSCOMMAND, SC_MINIMIZE, SW_FORCEMINIMIZE, SW_MINIMIZE,
 };
...
fn run_with_minimize_windows_title_substring_ci(needle: &str) -> std::io::Result<usize> {
 ...
 use windows::Win32::UI::WindowsAndMessaging::{
  EnumWindows, GetWindow, GetWindowTextW, GetWindowThreadProcessId, GW_OWNER, IsWindowVisible,
  WNDENUMPROC,
 };
...
mod tests {
	...
	fn distributed_conf_toml_parses_and_has_no_processors() {
		...
		let text = std::fs::read_to_string(&path)
			.unwrap_or_else(|e| panic!("conf.toml を読めない: {} ({e})", path.display()));
		let conf: Conf = toml::from_str(&text)
			.unwrap_or_else(|e| panic!("conf.toml の TOML パースに失敗: {e}"));
		assert!(
