...
fn type_err(port: &str, expected: SocketType, actual: SocketType) -> NodeExecError {
 NodeExecError::TypeMismatch { port: port.into(), expected, actual }
}
...
pub fn get_required_json<'a>(
 inputs: &'a InputMap,
 key: &str,
) -> Result<&'a serde_json::Value, NodeExecError> {
 let v = inputs.get(key).ok_or_else(|| NodeExecError::MissingRequiredInput(key.into()))?;
 v.as_json().map_err(|_| type_err(key, SocketType::Json, v.type_of()))
}

pub fn get_required_list<'a>(
 inputs: &'a InputMap,
 key: &str,
) -> Result<&'a [SocketValue], NodeExecError> {
 let v = inputs.get(key).ok_or_else(|| NodeExecError::MissingRequiredInput(key.into()))?;
 v.as_list()
  .map_err(|_| type_err(key, SocketType::List(Box::new(SocketType::Json)), v.type_of()))
}

pub fn get_required_map<'a>(
 inputs: &'a InputMap,
 key: &str,
) -> Result<&'a std::collections::BTreeMap<String, SocketValue>, NodeExecError> {
 let v = inputs.get(key).ok_or_else(|| NodeExecError::MissingRequiredInput(key.into()))?;
 v.as_map()
...
pub fn get_optional_string(
 ...
 key: &str,
 default: &str,
) -> Result<String, NodeExecError> {
 match inputs.get(key) {
  None => Ok(default.to_owned()),
  Some(v) => v.as_str().map(str::to_owned).map_err(|_| type_err(key, SocketType::String, v.type_of())),
 }
}
...
impl TriggerEvent {
 pub fn new(node_id: impl Into<String>) -> Self {
  Self { node_id: node_id.into(), fired_exec: Vec::new(), data_overrides: InputMap::new() }
 }
...
pub enum NodeExecError {
 ...
 TypeMismatch { port: String, expected: SocketType, actual: SocketType },
