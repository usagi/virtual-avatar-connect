...
mod tests {
	...
	#[tokio::test]
	async fn state_accumulator_snapshot_declared_test_passes() {
		let dir = example_dir("state-accumulator");
		let report = run_fixture_once_report(&dir).await.expect("report");
		assert!(report.ok, "report: {:?}", report.tests);
		assert_eq!(report.state_restore.restored_node_count, 1);
		assert_eq!(report.state_restore.nodes[0].node, "main::acc");
		assert_eq!(report.state_restore.nodes[0].version, 9);
		assert_eq!(report.state_versions, vec![("main::acc".into(), 10)]);
		assert_eq!(report.state_snapshots.len(), 1);
		assert_eq!(report.state_snapshots[0].node, "main::acc");
		assert_eq!(report.state_snapshots[0].format, "json");
		assert_eq!(
			report.state_snapshots[0].value,
	...
	#[tokio::test]
	async fn state_latch_snapshot_declared_test_passes() {
		let dir = example_dir("state-latch");
		let report = run_fixture_once_report(&dir).await.expect("report");
		assert!(report.ok, "report: {:?}", report.tests);
		assert_eq!(report.state_restore.restored_node_count, 1);
		assert_eq!(report.state_restore.nodes[0].node, "main::latch");
		assert_eq!(report.state_restore.nodes[0].version, 7);
		assert_eq!(report.state_versions, vec![("main::latch".into(), 8)]);
		assert_eq!(report.state_snapshots.len(), 1);
		assert_eq!(report.state_snapshots[0].node, "main::latch");
		assert_eq!(report.state_snapshots[0].format, "json");
		assert_eq!(
			report.state_snapshots[0].value,
			serde_json::json!({ "has_value": true, "value": { "message": "updated", "count": 2 } })
		);
		assert_eq!(report.failed_tests, 0);
	}
