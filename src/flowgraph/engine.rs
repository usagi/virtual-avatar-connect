...
impl FlowgraphBuilder {
	...
	pub fn add_node(&mut self, id: impl Into<NodeId>, impl_: NodeImpl, properties: InputMap) -> &mut Self {
		let id = id.into();
...
impl FlowgraphProgram {
	...
	pub async fn execute(&mut self, ctx: &mut ExecCtx) -> Result<ProgramRun, NodeExecError> {
		...
		while let Some((node_id, fired, overrides)) = queue.pop_front() {
			self.fire_node(&node_id, fired, overrides, generation, ctx, &mut run, &mut queue)
				.await?;
		}
	...
	pub async fn run_forever<F>(&mut self, ctx: &mut ExecCtx, shutdown: F) -> Result<ProgramRun, NodeExecError>
	where
		F: Future<Output = ()>,
	{
	...
	async fn handle_trigger(&mut self, event: TriggerEvent, ctx: &mut ExecCtx, run: &mut ProgramRun) -> Result<(), NodeExecError> {
		...
		let result = match &mut node.impl_ {
			...
			NodeImpl::Stateful { node: sn, state } => {
				let sctx = StatefulCtx {
					node_id,
					trigger: trigger_handle_opt.as_ref(),
				};
...
fn build_program(raw_nodes: Vec<NodeInstance>, raw_edges: Vec<Edge>) -> Result<FlowgraphProgram, BuildError> {
	...
	for e in &raw_edges {
		let from_node = nodes
			.get(&e.from.node)
			.ok_or_else(|| BuildError::UnknownNode(e.from.node.clone()))?;
...
fn detect_cycles_exec(nodes: &HashMap<NodeId, NodeInstance>, exec_succ: &HashMap<PortRef, Vec<PortRef>>) -> Result<(), BuildError> {
	let mut preds: HashMap<NodeId, HashSet<NodeId>> = HashMap::new();
...
fn dfs_cycle(cur: &NodeId, preds: &HashMap<NodeId, HashSet<NodeId>>, visited: &mut HashSet<NodeId>, stack: &mut HashSet<NodeId>) -> bool {
...
fn coerce_input(node_id: &NodeId, port_name: &str, target_ty: &SocketType, value: SocketValue) -> Result<SocketValue, NodeExecError> {
	coerce_to_type(value, target_ty)
		.map_err(|e| NodeExecError::Generic(anyhow::anyhow!("ノード '{}' の入力 '{}' の coerce 失敗: {}", node_id, port_name, e)))
}
...
mod tests {
	...
	async fn log_receives_quantity_as_formatted_string() {
		...
		assert!(
			...
			"unit (m/s^2 canonical form) present: {line}"
		);
	}
