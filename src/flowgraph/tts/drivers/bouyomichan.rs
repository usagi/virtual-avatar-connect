...
impl TtsDriver for BouyomichanDriver {
 ...
 fn params_schema(&self) -> TtsParamSchema {
  TtsParamSchema {
   entries: vec![
    TtsParamEntry { key: "speed_raw", ty: TtsParamType::Int, description: "iSpeed 直接指定 (-1 or 50..=300)" },
    TtsParamEntry { key: "tone_raw", ty: TtsParamType::Int, description: "iTone 直接指定 (-1 or 50..=200)" },
    TtsParamEntry { key: "volume_raw", ty: TtsParamType::Int, description: "iVolume 直接指定 (-1 or 0..=100)" },
    TtsParamEntry { key: "voice_raw", ty: TtsParamType::Int, description: "iVoice 直接指定 (0..=8)" },
   ],
  }
 }

 async fn speak(&self, req: TtsRequest, _audio: &AudioContext<'_>) -> Result<TtsOutcome, TtsError> {
  let endpoint = if req.endpoint.is_empty() { DEFAULT_ENDPOINT } else { req.endpoint.as_str() };

  let speed = extra_i64(&req.extra, "speed_raw").map(|v| v.clamp(-1, 300) as i16).unwrap_or_else(|| map_speed(req.speed));
  let tone = extra_i64(&req.extra, "tone_raw").map(|v| v.clamp(-1, 200) as i16).unwrap_or_else(|| map_tone(req.pitch));
  let volume = extra_i64(&req.extra, "volume_raw").map(|v| v.clamp(-1, 100) as i16).unwrap_or_else(|| map_volume(req.volume));
  let voice = extra_i64(&req.extra, "voice_raw").map(|v| v.clamp(0, 8) as i16).unwrap_or_else(|| parse_voice(&req.voice));
  ...
  Ok(TtsOutcome { played: true, audio_path: String::new() })
 }
}
