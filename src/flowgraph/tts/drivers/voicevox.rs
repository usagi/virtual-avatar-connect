...
pub(super) async fn speak_voicevox_like(
	...
	let engine_base = if req.endpoint.is_empty() {
		engine_base_default
	} else {
		req.endpoint.as_str()
	};
...
async fn resolve_speaker(engine_base: &str, voice: &str) -> Result<i64, TtsError> {
	...
	if let Some((uuid, rest)) = v.split_once(':') {
		...
		v.parse::<i64>()
			.map_err(|_| TtsError::Config(format!("voice '{v}' を style_id (整数) or 'uuid:N' として解釈できません")))
	}
}
	...
	TtsParamSchema {
		entries: vec![
			TtsParamEntry {
				...
				description: "VOICEVOX intonationScale (default 1.0)",
			},
			TtsParamEntry {
			...
			TtsParamEntry {
				...
				ty: TtsParamType::Int,
				description: "outputSamplingRate (default 24000)",
			},
		],
	}
}
