...
use super::super::driver::{
	extra_f64, extra_i64, maybe_save_wav, AudioContext, TtsDriver, TtsError, TtsOutcome, TtsParamEntry, TtsParamSchema, TtsParamType,
	TtsRequest,
};
...
fn parse_voice(voice: &str) -> Result<(&str, i64), TtsError> {
	...
	let (uuid, rest) = v
		.split_once(':')
		.ok_or_else(|| TtsError::Config(format!("CoeiroInk の voice は 'uuid:style_id' 形式が必要です（got: '{v}'）")))?;
	let style_id = rest
		.trim()
		.parse::<i64>()
		.map_err(|_| TtsError::Config(format!("CoeiroInk の voice 末尾 style_id が整数ではありません: '{v}'")))?;
...
impl TtsDriver for CoeiroinkDriver {
	...
	fn params_schema(&self) -> TtsParamSchema {
		TtsParamSchema {
			entries: vec![
				TtsParamEntry {
					key: "intonation_scale",
					ty: TtsParamType::Float,
					description: "intonationScale (default 1.0)",
				},
				TtsParamEntry {
					key: "pre_phoneme_length",
					ty: TtsParamType::Float,
					description: "prePhonemeLength (default 0.1)",
				},
				TtsParamEntry {
					key: "post_phoneme_length",
					ty: TtsParamType::Float,
					description: "postPhonemeLength (default 0.1)",
				},
				TtsParamEntry {
					key: "output_sampling_rate",
					ty: TtsParamType::Int,
					description: "outputSamplingRate (default 48000)",
				},
			],
		}
	}
