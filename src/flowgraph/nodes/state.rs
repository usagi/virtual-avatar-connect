...
fn socket_value_to_json(v: &SocketValue) -> JsonValue {
	match v {
		...
		SocketValue::Option(value) => value.as_ref().map(|value| socket_value_to_json(value)).unwrap_or(JsonValue::Null),
...
mod tests {
	...
	async fn bool_state_set_and_toggle() {
		...
		// no firing → data のみ返す
		let out = node
			.compute(state.as_mut(), &empty, &empty, &ExecFireSet::new(), &sctx)
			.await
			.unwrap();
		assert_eq!(out.data.get("value"), Some(&SocketValue::Bool(false)));
	...
	async fn int_counter_increment_decrement_reset() {
		...
		let mut state: Box<dyn Any + Send> = node.init_state();
		let sctx = sctx();
		let inputs: InputMap = [("step".into(), SocketValue::Int(5)), ("reset_value".into(), SocketValue::Int(100))]
			.into_iter()
			.collect();

		let out = node
			.compute(state.as_mut(), &InputMap::new(), &inputs, &fire("increment"), &sctx)
			.await
			.unwrap();
		...
		let out = node
			.compute(state.as_mut(), &InputMap::new(), &inputs, &fire("decrement"), &sctx)
			.await
			.unwrap();
		assert_eq!(out.data.get("value"), Some(&SocketValue::Int(5)));
		let out = node
			.compute(state.as_mut(), &InputMap::new(), &inputs, &fire("reset"), &sctx)
			.await
			.unwrap();
	...
	async fn latch_holds_last_value() {
		...
		let inputs: InputMap = [("input".into(), SocketValue::Json(serde_json::json!("hello")))]
			.into_iter()
			.collect();
		let out = node
