...
use crate::flowgraph::node::{
 get_optional_bool, get_required_json, get_required_string, ExecFireSet, InputMap, NodeDescriptor,
...
mod tests {
 ...
 async fn parse_and_stringify_roundtrip() {
  ...
  let out = JsonStringifyNode.compute(&InputMap::new(), &inputs, &ExecFireSet::new()).await.unwrap();
 ...
 async fn get_by_dot_path() {
  ...
  let inputs: InputMap =
   [("value".into(), value.clone()), ("path".into(), SocketValue::String("a.b[1]".into()))].into_iter().collect();
  ...
  let inputs: InputMap =
   [("value".into(), value.clone()), ("path".into(), SocketValue::String("a.c".into()))].into_iter().collect();
  let out = JsonGetNode.compute(&InputMap::new(), &inputs, &ExecFireSet::new()).await.unwrap();
  assert_eq!(out.data.get("result"), Some(&SocketValue::Json(serde_json::json!("hi"))));

  let inputs: InputMap =
   [("value".into(), value), ("path".into(), SocketValue::String("a.missing".into()))].into_iter().collect();
  let out = JsonGetNode.compute(&InputMap::new(), &inputs, &ExecFireSet::new()).await.unwrap();
 ...
 async fn parse_invalid_errors() {
  ...
  let e = JsonParseNode.compute(&InputMap::new(), &inputs, &ExecFireSet::new()).await.unwrap_err();
