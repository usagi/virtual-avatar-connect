	...
	fn describe(&self) -> NodeSpec {
		NodeSpec {
			...
			inputs: vec![
				...
				PortSpec::input("target_title", "Target Title", SocketType::String).with_default(SocketValue::String(String::new())),
...
impl EffectfulNode for ScreenshotCaptureNode {
	async fn execute(
		...
		#[cfg(target_os = "windows")]
		{
			...
			let using = if use_bitblt {
				winss::Using::BitBlt
			} else {
				winss::Using::PrintWindow
			};
				...
				match regex::Regex::new(&target_title_regex) {
					Ok(re) => match win_screenshot::utils::window_list() {
						Ok(list) => list
							...
							.unwrap_or_else(|| {
								Err(format!(
									"target_title_regex {target_title_regex:?} にマッチするウィンドウがありません"
								))
							}),
