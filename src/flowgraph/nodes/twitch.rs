...
use crate::flowgraph::node::{
 get_optional_int, get_optional_string, get_required_int, get_required_string, EffectfulNode, ExecCtx, ExecFireSet,
...
fn strip_substrings(inputs: &InputMap) -> Vec<String> {
 let Some(v) = inputs.get("strip_substrings") else { return Vec::new() };
 let Ok(list) = v.as_list() else { return Vec::new() };
 list.iter().filter_map(|e| e.as_str().ok().map(str::to_owned)).filter(|s| !s.is_empty()).collect()
}
...
impl NodeDescriptor for GetTokenNode {
 fn describe(&self) -> NodeSpec {
  NodeSpec {
   ...
   description: Some(
    "conf.twitch で定義された token_key から保存済み OAuth トークンを取り出し、on_success / on_failure で分岐する"
     .into(),
   ),
   inputs: vec![
...
impl NodeDescriptor for ValidateTokenNode {
 fn describe(&self) -> NodeSpec {
  NodeSpec {
   ...
   category: "twitch".into(),
   description: Some(
    "GET https://id.twitch.tv/oauth2/validate で token 所有者の user_id / login / client_id を取得する".into(),
   ),
   inputs: vec![
    PortSpec::exec_input("exec_in", "Exec"),
...
impl EffectfulNode for ValidateTokenNode {
 async fn execute(
  ...
  let url = if endpoint.trim().is_empty() { VALIDATE_URL.to_string() } else { endpoint.trim().to_string() };
  ...
  let resp = match client.get(&url).header("Authorization", format!("Bearer {}", token.trim())).send().await {
   Ok(r) => r,
...
impl EffectfulNode for UserIdByLoginNode {
 async fn execute(
  ...
  let resp = match client.get(&url).header("Client-Id", client_id).bearer_auth(token.trim()).send().await {
   Ok(r) => r,
...
async fn execute_moderation_ban(
 ...
 if let Some(d) = duration_secs {
  if !(1..=TIMEOUT_MAX_SECS).contains(&d) {
   return Ok(moderation_err(format!("duration_secs は 1..={TIMEOUT_MAX_SECS} の範囲でなければなりません（指定値: {d}）")));
  }
...
impl NodeDescriptor for TimeoutNode {
 fn describe(&self) -> NodeSpec {
  NodeSpec {
   ...
   category: "twitch".into(),
   description: Some(
    "Helix POST /moderation/bans（時間制限 ban）。`duration_secs` は 1..=1209600（最大 2 週間）。永久 ban は `twitch.ban`".into(),
   ),
...
mod tests {
 ...
 fn strip_substrings_extracts_and_filters_empty() {
  ...
  m.insert("strip_substrings".into(), SocketValue::List(vec![
 ...
 async fn get_token_empty_key_fires_on_failure() {
  ...
  let err = out.data.get("error").and_then(|v| v.as_str().ok().map(str::to_owned)).unwrap_or_default();
 ...
 async fn chat_send_empty_after_strip_fires_on_skipped() {
  ...
  inputs.insert("strip_substrings".into(), SocketValue::List(vec![
   SocketValue::String("aa".into()),
   SocketValue::String("BB".into()),
  ]));
  let out = n.execute(&mut ctx, &InputMap::new(), &inputs, &fired("exec_in")).await.unwrap();
 ...
 async fn ban_noop_without_fire() {
  let n = BanNode;
  let mut ctx = ExecCtx::default();
  let out = n.execute(&mut ctx, &InputMap::new(), &ban_inputs_full(), &ExecFireSet::new()).await.unwrap();
  assert!(out.fired_exec.is_empty());
 }
 ...
 async fn ban_unreachable_endpoint_is_non_fatal() {
  let n = BanNode;
  let mut ctx = ExecCtx::default();
  let out = n.execute(&mut ctx, &InputMap::new(), &ban_inputs_full(), &fired("exec_in")).await.unwrap();
...
mod integration_tests {
 ...
 async fn twitch_ingress_gated_by_rate_limit_then_chat_send_branch() {
  ...
  prog.run_forever_with_bus(&mut ctx, handle, rx, shutdown).await.expect("run_forever_with_bus");
  sender.await.unwrap();
