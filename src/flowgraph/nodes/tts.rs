...
use crate::flowgraph::node::{
 get_optional_float, get_optional_map, get_optional_string, get_required_string, EffectfulNode, ExecCtx, ExecFireSet,
...
impl NodeDescriptor for TtsSpeakNode {
 fn describe(&self) -> NodeSpec {
  NodeSpec {
   ...
   category: "tts".into(),
   description: Some(
    "engine 入力で選んだ TTS ドライバに text を合成させ、VAC 共有 audio_sink で再生する。save_path 指定時は WAV も保存。".into(),
   ),
   inputs: vec![
...
impl EffectfulNode for TtsSpeakNode {
 async fn execute(
  ...
  let audio = AudioContext { sink: ctx.audio_sink.as_ref() };
...
mod tests {
 ...
 async fn unknown_engine_fires_on_error() {
  ...
  let out = n.execute(&mut ctx, &InputMap::new(), &inputs_with("nonexistent", "hi"), &fired).await.unwrap();
