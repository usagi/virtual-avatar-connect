...
use crate::flowgraph::node::{
 get_required_int, get_required_list, get_required_map, get_required_string, ExecFireSet, InputMap,
...
mod tests {
 ...
 async fn list_ops() {
  ...
  let inputs: InputMap = [("items".into(), items.clone()), ("index".into(), SocketValue::Int(1))].into_iter().collect();
  ...
  let inputs: InputMap = [("items".into(), items), ("index".into(), SocketValue::Int(99))].into_iter().collect();
  let out = ListGetNode.compute(&InputMap::new(), &inputs, &ExecFireSet::new()).await.unwrap();
  ...
  let out = ListIsEmptyNode.compute(&InputMap::new(), &inputs, &ExecFireSet::new()).await.unwrap();
 ...
 async fn map_ops() {
  ...
  let inputs: InputMap = [("m".into(), m.clone()), ("key".into(), SocketValue::String("a".into()))].into_iter().collect();
  ...
  let inputs: InputMap = [("m".into(), m.clone()), ("key".into(), SocketValue::String("missing".into()))].into_iter().collect();
  let out = MapGetNode.compute(&InputMap::new(), &inputs, &ExecFireSet::new()).await.unwrap();
  ...
  assert_eq!(
   out.data.get("keys"),
   Some(&SocketValue::List(vec![SocketValue::String("a".into()), SocketValue::String("b".into())]))
  );

  let inputs: InputMap = [("m".into(), m), ("key".into(), SocketValue::String("b".into()))].into_iter().collect();
