	...
	fn describe(&self) -> NodeSpec {
		NodeSpec {
			...
			inputs: vec![
				...
				PortSpec::input("check_result_lang", "Check Result Lang", SocketType::Bool).with_default(SocketValue::Bool(false)),
			],
			outputs: vec![
			...
			if check_result_lang {
				if let Some(info) = whatlang::detect(&text) {
					...
					match estimated_l2 {
						...
						Some(l2) => {
							return Ok(err_output(format!("OCR 結果の言語推定が不一致: estimated={l2} ref={ref_l2}")));
						}
						None => {
							return Ok(err_output(format!("OCR 結果の推定言語コード変換に失敗: {estimated_l3}")));
						}
					}
				} else {
