...
use crate::flowgraph::node::{
	get_optional_int, get_required_int, InputMap, NodeDescriptor, NodeExecError, NodeOutput, NodeSpec, PortSpec, StatefulCtx, StatefulNode,
	TriggerEvent,
};
	...
	fn describe(&self) -> NodeSpec {
		NodeSpec {
			...
			inputs: vec![
				...
				PortSpec::input("value", "Value", SocketType::Json).with_default(SocketValue::Json(serde_json::Value::Null)),
...
impl StatefulNode for DelayNode {
	...
	async fn compute(
		...
		if fired_exec.contains("exec_in") {
			let value = inputs.get("value").cloned().unwrap_or(SocketValue::Json(serde_json::Value::Null));
		...
		if fired_exec.contains("__resume__") {
			...
			let value = state
				.pending
				.remove(&id)
				.ok_or_else(|| NodeExecError::Generic(anyhow::anyhow!("Delay resume: pending id {} not found (already consumed?)", id)))?;
