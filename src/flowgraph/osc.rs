...
//!
//! [`super::nodes::osc_send::OscSendNode`] や将来の VMC pose 送出などが利用する。
//! [`crate::motion::vmc_osc`] の **受信デコード**とは役割が異なる。
...
mod tests {
	...
	fn json_to_osc_int_and_double() {
		assert!(matches!(
			json_value_to_osc_type(&json!(42)).unwrap(),
			OscType::Int(42)
		));
		assert!(matches!(
			json_value_to_osc_type(&json!(1.5)).unwrap(),
			OscType::Double(_)
		));
	}
