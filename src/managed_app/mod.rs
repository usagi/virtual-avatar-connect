...
impl ManagedAppRegistry {
	...
	pub fn from_run_with(run_with: &[RunWith]) -> Self {
		...
		let statuses = specs.iter().map(|s| (s.id.clone(), ManagedAppStatus::unknown(&s.id))).collect();
		Self { specs, statuses }
	}
...
pub async fn run_monitor(
	...
	loop {
		...
		for s in changed {
			log::debug!("《ManagedApp》 状態変化 id={:?} running={} pids={:?}", s.id, s.running, s.pids);
...
fn send_close_to_pids(pids: &std::collections::HashSet<u32>, method: RunWithShutdownMethod) -> usize {
	...
	use windows::Win32::UI::WindowsAndMessaging::{
		EnumWindows, GetWindow, GetWindowThreadProcessId, IsWindowVisible, PostMessageW, SendMessageTimeoutW, GW_OWNER, SC_CLOSE,
		SMTO_ABORTIFHUNG, SMTO_NORMAL, WM_CLOSE, WM_SYSCOMMAND,
	};
