//! `gui/dist` をビルド時に [`include_dir`] で取り込み、`/gui/*` をメモリから返す（feature `embed-gui`）。
//!
...
static GUI_DIST: Dir<'static> = include_dir!("gui/dist");
...
async fn serve_embedded(req: HttpRequest) -> impl Responder {
	...
	let Some(file) = GUI_DIST.get_file(&rel) else {
