...
pub async fn post_oauth_cancel(
 state: Data<SharedState>,
 path: web::Path<String>,
) -> impl Responder {
 let Some(account) = OAuthAccount::from_path(&path.into_inner()) else {
...
pub async fn delete_oauth_tokens(
 ...
 let deleted_path = match oauth::delete_stored_tokens(&ident) {
  ...
  Err(e) => {
   log::error!(
    "[ControlAPI/oauth] tokens delete {} failed: {:#}",
    account.as_tag(),
    e
   );
...
async fn start_oauth(state: &SharedState, account: OAuthAccount) -> Result<OAuthStartResponse> {
 ...
 let (sessions, tx, ident) = {
  ...
  let ident = match account {
   OAuthAccount::Broadcaster => OAuthIdent::for_broadcaster(es),
   OAuthAccount::Moderator => {
    let mc = twitch.moderator.as_ref().ok_or_else(|| {
     anyhow!("[twitch.moderator] section is not configured; add it to use the moderator account")
    })?;
    OAuthIdent::for_moderator(es, mc)
   },
  };
 ...
 let handle = tokio::spawn(async move {
  let result =
   poll_device_token(&ident_for_task.client_id, &ident_for_task.scopes, &device_code, interval_secs, deadline_instant)
    .await;
