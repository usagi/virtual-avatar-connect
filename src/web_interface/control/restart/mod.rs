...
use util::{label_from_filename, paths_equivalent, resolve_new_conf_path};
...
#[post("/restart")]
pub async fn post_restart(state: Data<SharedState>, body: Json<RestartRequest>) -> impl Responder {
	let req = body.into_inner();
	let graceful_ms = req.graceful_ms.unwrap_or(DEFAULT_GRACEFUL_MS).min(MAX_GRACEFUL_MS);
	...
	let current_exe = match std::env::current_exe() {
		Ok(p) => p,
		Err(e) => {
			return HttpResponse::InternalServerError().json(serde_json::json!({ "error": format!("current_exe() failed: {e}") }));
		}
	};
...
mod tests {
	...
	fn restart_request_partial() {
		let parsed: RestartRequest = serde_json::from_str(r#"{"conf":"conf.example-twitch.toml"}"#).unwrap();
