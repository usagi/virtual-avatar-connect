...
impl WebInputRegistry {
 pub fn from_conf(conf: &Conf) -> anyhow::Result<Self> {
  ...
  for pc in &conf.processors {
   ...
   if !matches!(
    pc.feature.as_deref().map(|s| s.eq_ignore_ascii_case("webinput")),
    Some(true)
   ) {
    continue;
   }
...
async fn handle_input(
 ...
 let cd = ChannelDatum::new(channel.clone(), payload.content.clone())
  .with_flag_if(ChannelDatum::FLAG_IS_FINAL, payload.is_final);
